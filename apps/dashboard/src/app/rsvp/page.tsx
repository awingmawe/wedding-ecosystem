...
import { useRealtimeStats, type EventStats, type RsvpTrackingItem } from '@/hooks/use-realtime-stats';
...
function StatCard({ label, value, color }: { label: string; value: number; color?: string }) {
  return (
    <div className="rounded-xl bg-white p-5 shadow-sm">
      ...
      <p className={`mt-1 text-2xl font-bold ${color || 'text-primary'}`}>
        {value}
      </p>
    </div>
  );
...
export default function RsvpTrackingPage() {
  // TODO: Get eventId from user context/selected event
  // For now, use a placeholder that would come from auth context or route
  ...
  useEffect(() => {
    async function fetchInitialData() {
      try {
        // Fetch current event for the user
        const eventData = await apiFetch<{ id: string }>('/events/current');
        setEventId(eventData.id);

        // Fetch initial stats
        const statsData = await apiFetch<EventStats>(`/events/${eventData.id}/stats`);
        setInitialStats(statsData);

        // Fetch initial RSVP list
        const rsvpData = await apiFetch<{ data: RsvpTrackingItem[] }>(
          `/events/${eventData.id}/rsvp`
        );
        setInitialRsvpList(rsvpData.data);
      } catch {
        // If API is not available, use empty defaults
        // This allows the page to still render with WebSocket updates
      } finally {
  ...
  // WebSocket connection (Req 9.8)
  const { socket, connectionStatus } = useSocket({
    eventId,
    autoConnect: true,
  });

  // Real-time stats and RSVP list (Req 9.6)
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {/* Real-time statistics panel (Req 9.6) */}
...
/** Badge component for attendance type */
